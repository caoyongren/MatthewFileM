...
public class PropertyDialog extends Dialog {
    ...
    private void initBody(File file) {
        ...
        size.setText(Util.formatFileSize(file.length()));
        sizeOnDisk.setText(Util.formatFileSize(file.length()));
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
                if (line.contains("Access") && line.contains("Uid")) {
                    String limit = line.substring(Constants.INDEX_LIMIT_BEGIN,
                                                  Constants.INDEX_LIMIT_END);
                    initLimit(limit);
    ...
    private void initLimit(String line) {
        ...
        if (!TextUtils.isEmpty(line)) {
            limit = line.substring(0, Constants.LIMIT_LENGTH);
            if (limit.charAt(Constants.LIMIT_OWNER_READ) == 'r') {
                limitOwnerRead.setChecked(true);
            }
            if (limit.charAt(Constants.LIMIT_OWNER_WRITE) == 'w') {
                limitOwnerWrite.setChecked(true);
            }
            if (limit.charAt(Constants.LIMIT_OWNER_EXECUTE) == 'x') {
                limitOwnerExecute.setChecked(true);
            }
            if (limit.charAt(Constants.LIMIT_GROUP_READ) == 'r') {
                limitGroupRead.setChecked(true);
            }
            if (limit.charAt(Constants.LIMIT_GROUP_WRITE) == 'w') {
                limitGroupWrite.setChecked(true);
            }
            if (limit.charAt(Constants.LIMIT_GROUP_EXECUTE) == 'x') {
                limitGroupExecute.setChecked(true);
            }
            if (limit.charAt(Constants.LIMIT_OTHER_READ) == 'r') {
                limitOtherRead.setChecked(true);
            }
            if (limit.charAt(Constants.LIMIT_OTHER_WRITE) == 'w') {
                limitOtherWrite.setChecked(true);
            }
            if (limit.charAt(Constants.LIMIT_OTHER_EXECUTE) == 'x') {
